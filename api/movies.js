...
async function fetchWithRetry(url, retries = 3) {
  for (let attempt = 1; attempt <= retries; attempt++) {
    try {
      ...
      if (attempt === retries || !['ECONNRESET', 'ETIMEDOUT', 'ECONNABORTED', 'EPIPE'].includes(error.code)) throw error;
...
export default async function handler(req, res) {
  ...
  if (search && search.trim() !== '') {
    ...
    if (category === 'now_playing') {
      ...
      // 1. Set the default vote count (for general browsing)
      let minVotes = 200;
      ...
      // 2. If a specific language is chosen, lower the vote threshold
      // so we can discover movies from smaller film industries.
      if (language) {
        minVotes = 10; // Lower threshold for specific languages
      }
